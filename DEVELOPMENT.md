...
**`waitForDrain()` polling:** Uses a 50 ms poll interval. Checks both `pendingDepth === 0` and `!mutex.isLocked()` — the depth alone is not sufficient because the currently executing operation has already decremented it.
